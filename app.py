...
async def main():
    ...
    st.markdown(f"- 最後重置: {datetime.fromtimestamp(st.session_state.last_reset):%Y-%m-%d %H:%M:%S}")
    st.markdown(f"- 速率限制解除: {datetime.fromtimestamp(st.session_state.rate_limit_until):%Y-%m-%d %H:%M:%S if st.session_state.rate_limit_until > time.time() else '無限制'}")
    ...
    if user_question and not st.session_state.awaiting_response:
        ...
        try:
            ...
            if time.time() < st.session_state.rate_limit_until:
                error_message = f"速率限制中，請在 {datetime.fromtimestamp(st.session_state.rate_limit_until):%Y-%m-%d %H:%M:%S} 後重試。"
            ...
            with st.chat_message("assistant"):
                ...
                async for chunk in stream_grok3_response(
                    user_query=user_question,
                    metadata=[{"thread_id": item["thread_id"], "title": item["title"]} for item in result.get("thread_data", [])],
            ...
            error_message = f"系統錯誤：{str(e)}。請稍後重試。"
