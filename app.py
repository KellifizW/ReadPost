    ...
    if user_question and not st.session_state.awaiting_response:
        ...
        with st.spinner("正在處理..."):
            try:
                ...
                if not st.session_state.last_user_query or len(set(user_question.split()).intersection(set(st.session_state.last_user_query.split()))) < 2:  # 硬編碼：建議可配置
                ...
                post_limit = analysis.get("post_limit", 2)  # 硬編碼：建議允許用戶配置
                ...
                if analysis_advanced.get("needs_advanced_analysis"):
                    ...
                    if thread_data_advanced:
                        ...
                        grok_container.markdown(response)
                        logger.warning(f"Advanced analysis failed: {analysis_advanced.get('reason', 'No data fetched')}")
