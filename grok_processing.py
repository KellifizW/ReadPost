...
"""
    ...
    payload = {"model": "grok-3" if ai_engine == "grok3" else "gpt-3.5-turbo", "messages": [{"role": "user", "content": prompt}], "max_tokens": 100, "temperature": 0.5}
    async with aiohttp.ClientSession() as session:
        try:
            async with session.post(api_url, headers=headers, json=payload, timeout=API_TIMEOUT) as response:
                if response.status != 200:
                    logger.warning(f"對話摘要失敗：狀態碼={response.status}, 引擎={ai_engine}")
                    return {"theme": "一般", "keywords": []}
                data = await response.json()
                result = json.loads(data["choices"][0]["message"]["content"])
                logger.info(f"對話摘要成功：result={result}, 引擎={ai_engine}")
                return result
        except Exception as e:
            logger.warning(f"對話摘要錯誤：{str(e)}, 引擎={ai_engine}")
            return {"theme": "一般", "keywords": []}
...
async def analyze_and_screen(user_query, source_name, source_id, source_type="lihkg", conversation_context=None):
    ...
    try:
        ...
        return {
            ...
            "intents": [{"intent": "general_query", "confidence": 0.5, "reason": f"缺少 {ai_engine} API 密鑰"}],
            "theme": "一般",
            ...
            "reason": f"缺少 {ai_engine} API 密鑰",
            "theme_keywords": [],
        }
    logger.info(f"開始語義分析：查詢={user_query}, 引擎={ai_engine}")
    ...
    if not intents:
        ...
        logger.warning(f"無有效意圖：查詢={user_query}, 回退到 summarize_posts, 引擎={ai_engine}")
    ...
    logger.info(f"動態 post_limit 來自 parse_query: {post_limit}, 意圖: {primary_intent}, 引擎={ai_engine}")
    return {
...
async def prioritize_threads_with_grok(user_query, threads, source_name, source_id, source_type="lihkg", intents=["summarize_posts"], post_limit=5):
    ...
    try:
        ...
        logger.error(f"缺少 {ai_engine} API 密鑰")
        return {"top_thread_ids": [], "reason": f"缺少 {ai_engine} API 密鑰", "intent_breakdown": []}
    ...
    if any(intent == "follow_up" for intent in intents):
        ...
        if valid_ids:
            ...
            logger.info(f"檢測到追問意圖，使用參考帖子 ID: {valid_ids}, 引擎={ai_engine}")
...
"""
    ...
    logger.info(f"創建提示：長度={prompt_length} 字符, 估計 token={estimated_tokens}, 提示預覽={prompt[:200]}..., 引擎={ai_engine}")
    
    headers = {"Content-Type": "application/json", "Authorization": f"Bearer {api_key}"}
    api_url = GROK3_API_URL if ai_engine == "grok3" else CHATANYWHERE_API_URL
    payload = {"model": "grok-3" if ai_engine == "grok3" else "gpt-3.5-turbo", "messages": [{"role": "user", "content": prompt}], "max_tokens": 500, "temperature": 0.7}
    logger.info(f"API payload: {json.dumps(payload, ensure_ascii=False)[:200]}..., 引擎={ai_engine}")
    ...
    async with aiohttp.ClientSession() as session:
        for attempt in range(2):
            try:
                async with session.post(api_url, headers=headers, json=payload, timeout=API_TIMEOUT) as response:
                    if response.status != 200:
                        logger.warning(f"API 調用失敗：狀態碼={response.status}, 嘗試={attempt + 1}, 引擎={ai_engine}")
                        continue
                    data = await response.json()
                    if not data.get("choices"):
                        logger.warning(f"API 調用失敗：無選擇，嘗試={attempt + 1}, 引擎={ai_engine}")
                        continue
                    response_content = data["choices"][0]["message"]["content"]
                    logger.info(f"原始 API 回應：{response_content[:500]}..., 引擎={ai_engine}")
                    try:
                        ...
                        logger.info(f"API 返回 top_thread_ids: {top_thread_ids}, 原因={result.get('reason', '無原因')}, 最終選取數量={len(top_thread_ids)}, 引擎={ai_engine}")
                        return {
                            "top_thread_ids": top_thread_ids,
                        ...
                        logger.error(f"JSON 解碼錯誤：{str(e)}, response_content={response_content[:200]}..., 引擎={ai_engine}")
                        continue
            except Exception as e:
                logger.error(f"API 調用錯誤：{str(e)}, 嘗試={attempt + 1}, 引擎={ai_engine}")
                if attempt < 1:
                ...
                logger.info(f"回退到熱門度排序：top_thread_ids={top_thread_ids}, 原因=API 失敗 ({str(e)}), 最終選取數量={len(top_thread_ids)}, 引擎={ai_engine}")
                return {
                    "top_thread_ids": top_thread_ids,
                    "reason": f"排序失敗（{str(e)}），回退到熱門度排序",
                    "intent_breakdown": [],
                }
    logger.warning(f"排序失敗，所有嘗試均失敗，引擎={ai_engine}")
    return {"top_thread_ids": [], "reason": "排序失敗，所有嘗試均失敗", "intent_breakdown": []}

async def stream_ai_response(user_query, metadata, thread_data, processing, selected_source, conversation_context=None, needs_advanced_analysis=False, reason="", filters=None, source_id=None, source_type="lihkg"):
    conversation_context = conversation_context or []
    ...
    api_url = GROK3_API_URL if ai_engine == "grok3" else CHATANYWHERE_API_URL
    payload = {
        "model": "grok-3" if ai_engine == "grok3" else "gpt-3.5-turbo",
        "messages": [
            {
                "role": "system",
...
async def process_user_question(user_query, selected_source, source_id, source_type="lihkg", analysis=None, request_counter=0, last_reset=0, rate_limit_until=0, conversation_context=None, progress_callback=None):
    ...
    try:
        ...
        return {
            "selected_source": selected_source,
            "thread_data": [],
            "rate_limit_info": [{"message": f"缺少 {ai_engine} API 密鑰"}],
            "request_counter": request_counter,
            "last_reset": last_reset,
            "rate_limit_until": rate_limit_until,
            "analysis": analysis,
    ...
    logger.info(f"處理查詢：查詢={user_query}, post_limit={post_limit}, 意圖={primary_intent}, 引擎={ai_engine}")
    ...
    try:
        if top_thread_ids and primary_intent in ["fetch_thread_by_id", "follow_up", "analyze_sentiment"]:
            ...
            logger.info(f"初始帖子數量: {len(initial_threads)}, 過濾後帖子數量: {len(filtered_items)}, post_limit={post_limit}, 引擎={ai_engine}")
            ...
            if primary_intent == "fetch_dates":
                ...
                if filtered_items:
                    ...
                    logger.info(f"優先排序結果: top_thread_ids={top_thread_ids}, 原因={prioritization.get('reason', '無原因')}, 最終選取數量={len(top_thread_ids)}, 引擎={ai_engine}")
                    valid_thread_ids = [tid for tid in top_thread_ids if str(tid) in [str(item["thread_id"]) for item in filtered_items]]
                    logger.info(f"驗證後的帖子 ID: valid_thread_ids={valid_thread_ids}, 原始 top_thread_ids={top_thread_ids}, 引擎={ai_engine}")
                    candidate_threads = [item for item in filtered_items if str(item["thread_id"]) in valid_thread_ids][:post_limit]
                    if not candidate_threads:
                        logger.warning(f"未找到匹配的帖子 ID: {top_thread_ids}, 回退到按熱門度排序, 引擎={ai_engine}")
            ...
            logger.info(f"候選帖子: 數量={len(candidate_threads)}, 帖子 ID={[item['thread_id'] for item in candidate_threads]}, post_limit={post_limit}, 引擎={ai_engine}")
            if not candidate_threads:
                logger.warning(f"未找到候選帖子: 查詢={user_query}, 來源={selected_source}, 引擎={ai_engine}")
                return {
                    "selected_source": selected_source,
                    "thread_data": [],
        ...
        if len(thread_data) < post_limit and primary_intent == "follow_up":
            ...
            logger.info(f"補充帖子: 數量={len(supplemental_threads)}, 帖子 ID={[item['thread_id'] for item in supplemental_threads]}, 引擎={ai_engine}")
        ...
        logger.info(f"最終 thread_data：數量={len(thread_data)}, 帖子 ID={[d['thread_id'] for d in thread_data]}, post_limit={post_limit}, 來源={selected_source}, 引擎={ai_engine}")
        return {
            "selected_source": selected_source,
        ...
        logger.error(
            ...
            f"candidate_threads_count={len(candidate_threads) if 'candidate_threads' in locals() else 0}, 引擎={ai_engine}"
        )
        return {
            "selected_source": selected_source,
            "thread_data": [],
