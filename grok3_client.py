...
async def stream_grok3_response(user_query, metadata, thread_data, processing) -> AsyncGenerator[str, None]:
    ...
    if processing == "emotion_focused_summary":
        prompt = f"""
        你是一個智能助手，任務是基於 LIHKG 數據總結與感動或溫馨相關的帖子內容，回答用戶問題。以繁體中文回覆，150-200 字，僅用提供數據。
        ...
        prompt = f"""
        你是一個智能助手，任務是基於 LIHKG 數據總結與幽默或搞笑相關的帖子內容，回答用戶問題。以繁體中文回覆，150-200 字，僅用提供數據。
        ...
        輸出格式：
        - 總結：150-200 字，突出幽默或搞笑內容。
        - 進階分析建議：是否需要更多數據或改變處理方式，說明理由。
        """
    elif processing == "professional_summary":
        prompt = f"""
        你是一個智能助手，任務是基於 LIHKG 數據總結與財經、時事等專業主題相關的帖子內容，回答用戶問題。以繁體中文回覆，150-200 字，僅用提供數據。
        ...
        輸出格式：
        - 總結：150-200 字，突出專業或數據驅動內容。
        - 進階分析建議：是否需要更多數據或改變處理方式，說明理由。
        """
    elif processing == "summarize":
        prompt = f"""
        你是一個智能助手，任務是基於 LIHKG 數據總結帖子內容，回答用戶問題。以繁體中文回覆，150-200 字，僅用提供數據。
